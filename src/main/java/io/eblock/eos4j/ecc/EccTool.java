...
public class EccTool {
    ...
    public static byte[] getTransactionHash(TxSign push) {
        // tx
        ByteBuffer bf = new ByteBuffer();
        ObjectUtils.writeBytes(push, bf);
        byte[] real = bf.getBuffer();
        ...
        return Sha.SHA256(real);
    }

    public static String getTransactionSignature(byte[] vrs) {
        byte[] checksum = Ripemd160.from(ByteUtils.concat(vrs, "K1".getBytes())).bytes();
        byte[] signatureString = ByteUtils.concat(vrs, ByteUtils.copy(checksum, 0, 4));
        return "SIG_K1_" + Base58.encode(signatureString);
    }

    public static String getAddressFromPublicKey(byte[] publicKeyByte) {
        byte[] hash160 = Ripemd160.from(publicKeyByte).bytes();
        byte[] checksum = new byte[4];
        System.arraycopy(hash160, 0, checksum, 0, 4);
        byte[] eosByte = new byte[publicKeyByte.length + 4];
        System.arraycopy(publicKeyByte, 0, eosByte, 0, publicKeyByte.length);
        System.arraycopy(checksum, 0, eosByte, publicKeyByte.length, 4);
        return "EOS" + Base58.encode(eosByte);
    }
